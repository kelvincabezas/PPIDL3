...
def display():
    ...
    if X is not None and y is not None:
        if len(X) > 1:
            ...
            st.markdown('<p style="color:#2F855A;">Estas curvas muestran cómo de bien nuestro modelo está aprendiendo a predecir el volumen de captura. Si el error de validación es cercano al error de entrenamiento, significa que el modelo es bastante preciso y no se está sobreajustando a los datos de entrenamiento.</p>', unsafe_allow_html=True)
